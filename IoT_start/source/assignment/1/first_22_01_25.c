...
int main(void)
{
	...
	{
		student_rank[i] = 11;												// 모든 랭크를 n+1로 한다(같거나를 이용하므로 ( ' >= ' 사용 안하면 똑같은 점수를 가진사람들이 모두 불이익을 본다 ))
		for (int j = 0; j < 10; j++)										
		{
	...
	while (1)				// 기능하나 끝나고 다시 메인메뉴로 돌아오기 ( -1을 입력하기 전까지 반복 )
	{
		...
		else if (select_menu_num == 4)
		{
			...
			while (1)
			{
				printf("조회에서 삭제할 학생의 번호를 입력해주세요. :");
				scanf("%d", &delete_student_num);

				if (delete_student_num > 0 && delete_student_num <= 10)
				{
					printf("%d 번 학생의 데이터를 조회에서 제외합니다.\n", delete_student_num);
					delete_student_list[delete_student_num-1] = 1;

					for (int i = 0; i < 10; i++)											// 석차 구하기(삭제했으니 그걸 다시 구해야함)
					{
						student_rank[i] = 11;												// 모든 랭크를 n+1로 한다(같거나를 이용하므로 - =을 사용 안하면 똑같은 점수를 가진사람들이 모두 불이익을 본다 )
						for (int j = 0; j < 10; j++)
						{
				...
				else if (delete_student_num < 0 || delete_student_num > 10)
				{
...
void print_init_title(void)
{
	...
	printf("\n\t\t === < 학생성적 관리프로그램 > === \t\t\n");
	printf("------------------------------------------------------------------------------------------\n\n");
	printf(" 1)학생등록 및 수정 \t2)성적등록 및 수정 \t3)조회 \t\t4)삭제 \n");
	printf(" \n\t\t\t* 종료하시려면 -1을 입력해주세요\n");
	printf("------------------------------------------------------------------------------------------\n\n");
...
{
	// 총점순으로 재정렬후 출력(석차순으로 출력) + 석차,총점,평균등을 추가 출력
	printf("\n---------------------------------------------------------------------------------------\n\n");
