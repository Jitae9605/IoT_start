...
int main1212312421131(void)
{
	...
	// 변수선언
	char Check_id[id_limit + 1] = { 0 };								// 입력받은 아이디 임시저장
	char Check_pw[pw_limit + 1] = { 0 };								// 입력받은 비밀번호 임시저장
	int check_for_success_to_login = 0;									// Check_id와 Check_pw의 값을 플레이어리스트의 각각과 비교해 쌍이 모두 일치하면 1
	...
	while (1)
	{
		...
		{
			if (strcmp(Check_id, allPlayerList[i].id) == 0 && strcmp(Check_pw, allPlayerList[i].pw) == 0)
			{
				system("cls");
				Curent_id_num = i;
				printf("\n\t === 환영합니다. %s 님 === \t\t\n", allPlayerList[i].id);
